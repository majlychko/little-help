#обновила код, теперь текст в подстановках не заменяется на неразрывники (особенно актуально в фичах недели), тестирую, возможны баги
import json
import re

def replace_space(text):
    #  Исключаем текст в фигурных скобках + используем регулярное выражение для поиска слов из 1 или 2 символов,
    pattern = r"(?<!\{)([^{])(\b\w{1,2}\b)\s(?![^}])"  
    result = re.sub(pattern, r"\1\2\\u00A0", text)
    # Заменяем короткое тире на неразрывное, за исключением текста в фигурных скобках "{{ }}"
    result = re.sub(r"(?<!\{)([^{])-(?![^}])", r"\1\\u2011", result)

    # для справки, мб можно и проще было описать
    # (?<!\{) - это негативное ретроспективное условие, которое исключает совпадение, если перед тире есть открывающая фигурная скобка "{". Это нужно для того, чтобы не изменять короткие тире, которые находятся внутри фигурных скобок.
    # ([^{}]) - это группа захвата, которая соответствует любому символу, кроме фигурных скобок. Таким образом, мы исключаем текст внутри фигурных скобок из замены.
    # -(?![^}]) - это совпадение с тире, за которым не следует фигурная скобка "}" (с помощью негативного просмотра впереда). Это нужно для того, чтобы не изменять короткие тире, которые находятся внутри фигурных скобок.
    ...
    #ниже идут продукты тинька, Артур сказал неразрывники ставить в них
